 ...
 IF OBJECT_ID('TEMPDB..#prep_data') IS NOT NULL DROP TABLE #prep_data
	 ...
	 SELECT
	    ...
	    ,agenyears
		,CASE
			WHEN agenmosyrscat = 0 THEN 0
			WHEN agenmosyrscat = 1 THEN 1
			WHEN dq_dobgtlatremdt = 1 OR dq_gt21dobtodtlatrem = 1 THEN 8888
			WHEN dq_missdob = 1 OR dq_missdtlatremdt = 1 THEN 9999
			ELSE agenyears + 1
			END AS agenmosyrs
		,CASE
			WHEN dodfcdt IS NULL THEN 7777
			WHEN dq_dobgtdodfcdt = 1 OR dq_gt21dobtodtdisch = 1 THEN 8888
			WHEN dq_missdob = 1 THEN 9999
		...
		,CASE 
			...
			WHEN dq_dobgtdodfcdt = 1 OR dq_gt21dobtodtdisch = 1 THEN 8888
			WHEN dq_missdob = 1 THEN 9999
			WHEN [dbo].[fnc_datediff_mos](dob, dodfcdt) BETWEEN 216 AND 263 THEN 6
			WHEN [dbo].[fnc_datediff_mos](dob, dodfcdt) BETWEEN 204 AND 216 THEN 5
			WHEN [dbo].[fnc_datediff_mos](dob, dodfcdt) BETWEEN 132 AND 204 THEN 4
			WHEN [dbo].[fnc_datediff_mos](dob, dodfcdt) BETWEEN 72 AND 132 THEN 3
			WHEN [dbo].[fnc_datediff_mos](dob, dodfcdt) BETWEEN 12 AND 72 THEN 2
			WHEN [dbo].[fnc_datediff_mos](dob, dodfcdt) BETWEEN 4 AND 12 THEN 1
			WHEN [dbo].[fnc_datediff_mos](dob, dodfcdt) BETWEEN 0 AND 4 THEN 0
		...
		,CASE
			...
			END AS agexmosyrs
		,IIF(disreasn IS NULL OR disreasn = 0, 7777, disreasn) AS disreasn1
		,CASE
			WHEN disreasn IS NULL OR disreasn = 0 THEN 7777
			WHEN disreasn IN (1, 2, 3, 5) THEN 1
			WHEN disreasn IN (4, 6, 7) THEN 2
			WHEN disreasn = 8 THEN 3
			END AS disreasn2
	 ...
	 FROM
		 (SELECT
			...
			,dq_totalrem1
			,CASE 
				WHEN dq_missdtlatremdt = 1 THEN 9999
				WHEN MONTH(latremdt) BETWEEN 10 AND 12 THEN YEAR(latremdt) + 1
				ELSE YEAR(latremdt)
				END AS entryyr
			,CASE 
				WHEN dodfcdt IS NULL THEN 7777
				WHEN MONTH(dodfcdt) BETWEEN 10 AND 12 THEN YEAR(dodfcdt) + 1
				ELSE YEAR(dodfcdt)
				END AS exityr
			,[dbo].[fnc_datediff_mos](dob, latremdt) AS agenmos
			,CASE
				WHEN [dbo].[fnc_datediff_mos](dob, latremdt) BETWEEN 216 AND 263 THEN 6
				WHEN [dbo].[fnc_datediff_mos](dob, latremdt) BETWEEN 204 AND 216 THEN 5
				WHEN [dbo].[fnc_datediff_mos](dob, latremdt) BETWEEN 132 AND 204 THEN 4
				WHEN [dbo].[fnc_datediff_mos](dob, latremdt) BETWEEN 72 AND 132 THEN 3
				WHEN [dbo].[fnc_datediff_mos](dob, latremdt) BETWEEN 12 AND 72 THEN 2
				WHEN [dbo].[fnc_datediff_mos](dob, latremdt) BETWEEN 4 AND 12 THEN 1
				WHEN [dbo].[fnc_datediff_mos](dob, latremdt) BETWEEN 0 AND 4 THEN 0
		 ...
		 FROM
			(SELECT
				...
				,timebetweenreports
				,CASE
					WHEN dodfcdt IS NULL THEN CASE
						WHEN timebetweenreports IS NULL OR (timebetweenreports >= 12 AND timebetweenreports < 888) THEN 1
							END
								END AS dq_dropped 
				,dq_idnomatchnext6mo = IIF(timebetweenreports IS NULL OR (timebetweenreports >= 12 AND timebetweenreports < 888), 1, 0)
					...
					,dq_missdisreasn = IIF(dodfcdt IS NOT NULL AND disreasn IS NULL, 1, 0)
			...
			FROM 
				(SELECT
					...
					,LEAD(dtreportbeg, 1, NULL) OVER (PARTITION BY childid ORDER BY dtreportbeg) AS next6moreport
...
ORDER BY
 ...
 IF OBJECT_ID('TEMPDB..#location') IS NOT NULL DROP TABLE #location
	 ...
	 SELECT
		...
		,hisorgin
		,CASE
			WHEN hisorgin = 1 THEN 5
			ELSE 
				CASE 
					WHEN (amiakn + asian + blkafram + hawaiipi + white) > 1 THEN 7
					ELSE
						CASE
							WHEN amiakn = 1 THEN 1
							WHEN asian = 1 THEN 2
							WHEN blkafram = 1 THEN 3
							WHEN hawaiipi = 1 THEN 4
							WHEN white = 1 THEN 6
							WHEN untodetm = 1 THEN 7
							ELSE 8
							END 
							END 
							END AS [raceeth]
	 ...
	 LEFT JOIN [dbo].[ref_lookup_county] AS co
		ON co.countyfips = pd.fipscode
	 LEFT JOIN [dbo].[ref_lookup_county_region] AS cr
		ON co.county_cd = cr.county_cd
